        ...
        private IEnumerator 并发构造()
        {
                            ...
                            case "LeftHand":
                                {
                                    ...
                                    汉化模块工具.修改字体尺寸(鼠标右键提示, 24);
                                    鼠标右键提示.font = 前置_资源加载器.单例.当前TMP字体;

                                    var m_subTextObjects = (TMP_SubMeshUI[])Traverse.Create(鼠标右键提示).Field("m_subTextObjects").GetValue();
                                    foreach (var child in m_subTextObjects)
                                    { if (child) { child.fontAsset = 前置_资源加载器.单例.当前TMP字体; } }
                                }
                                break;
                            case "RightHand":
                                {
            ...
            if (前置_资源加载器.单例.逻辑组件字典.TryGetValue(typeof(Tooltip), out var 工具及鼠标左键提示))
            {
                foreach (Tooltip tooltip in 工具及鼠标左键提示)
                {
                    ...
                    var m_subTextObjects = (TMP_SubMeshUI[])Traverse.Create(鼠标左键提示).Field("m_subTextObjects").GetValue();
                    foreach (var child in m_subTextObjects)
                    { if (child) { child.fontAsset = 前置_资源加载器.单例.当前TMP字体; } }
