...
public class 在Unity编辑器中创建缩略图_生成_输入窗口 : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("创建缩略图", GUILayout.Height(60)))
        {
            ...
            在Unity编辑器中创建缩略图_生成.创建缩略图(物体名称, color);
            Close();
        }
...
public class 在Unity编辑器中创建缩略图_生成 : EditorWindow
{
    [MenuItem("Tools/在Unity编辑器中创建缩略图_生成")]
    static void Init()
    {
        var window = GetWindow<在Unity编辑器中创建缩略图_生成_输入窗口>(true, "输入物体名称");
        window.Show();
    }
    ...
    public static void 创建缩略图(string 物体名称, UnityEngine.Color 查找该颜色并替换成喷漆颜色)
    {
        Texture2D 母版 = null;
        ...
        if (母版 == null)
        {
            UnityEngine.Debug.Log($"创建缩略图时, 需要提供一张纹理图片作为母版, 在本函数中会复制母版并创建新的图片, 然后读取所有喷漆像素替换成对应的喷漆颜色");
            return;
        }
        ...
        var 所有喷漆颜色的缩略图_纹理 = 游戏内置喷漆颜色_生成.创建所有喷漆纹理图片(母版, 是否存在多分辨率预存, 查找该颜色并替换成喷漆颜色);

        foreach (var 当前颜色 in 游戏内置喷漆颜色_生成.所有喷漆颜色)
        {
            var 当前颜色depth = (int)当前颜色;
            var 当前纹理 = 所有喷漆颜色的缩略图_纹理[当前颜色depth];
        ...
        for (var i = 0; i < 所有喷漆颜色的缩略图_纹理.Length; ++i)
        {
            var 输出纹理路径 = Path.Combine(输出纹理目录, $"{游戏内置喷漆颜色_生成.所有喷漆颜色[i]}.png");
            File.WriteAllBytes(输出纹理路径, 所有喷漆颜色的缩略图_纹理[i].EncodeToPNG());
        }
