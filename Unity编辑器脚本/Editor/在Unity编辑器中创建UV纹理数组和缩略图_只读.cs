...
public class 在Unity编辑器中创建UV纹理数组和缩略图_只读_输入窗口 : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("创建UV纹理数组和缩略图", GUILayout.Height(60)))
        {
            ...
            在Unity编辑器中创建UV纹理数组和缩略图_只读.创建UV纹理数组和缩略图(物体名称);
            Close();
        }
...
public class 在Unity编辑器中创建UV纹理数组和缩略图_只读 : EditorWindow
{
    [MenuItem("Tools/在Unity编辑器中创建UV纹理数组和缩略图_只读")]
    static void Init()
    {
        var window = GetWindow<在Unity编辑器中创建UV纹理数组和缩略图_只读_输入窗口>(true, "输入物体名称");
        window.Show();
    }
    ...
    public static void 创建UV纹理数组和缩略图(string 物体名称)
    {
        ...
        if (游戏内置喷漆颜色_只读.所有喷漆颜色.Any(d => d == 颜色) && 纹理 != null)
        {
            ...
            var UV纹理数组 = new Texture2DArray(纹理.width, 纹理.height, 游戏内置喷漆颜色_只读.所有喷漆颜色.Count(), 纹理.format, 是否存在多分辨率预存);
            ...
            foreach (var 当前 in 游戏内置喷漆颜色_只读.所有喷漆颜色)
            {
                ...
                if (所有纹理.TryGetValue(当前, out var 当前纹理))
                {
                    if (是否存在多分辨率预存)
                    {
                        ...
                        {
                            UnityEngine.Graphics.CopyTexture(src: 当前纹理, srcElement: 0, srcMip: i, dst: UV纹理数组, dstElement: 当前depth, dstMip: i);
                        }
                    }
                    else
                    {
                        UnityEngine.Graphics.CopyTexture(src: 当前纹理, srcElement: 0, dst: UV纹理数组, dstElement: 当前depth);
                    }
                ...
                else
                {
                    ...
                    if (是否存在多分辨率预存)
                    {
                        for (var i = 0; i < 多分辨率预存计数; ++i)
                        {
                            UnityEngine.Graphics.CopyTexture(src: UV纹理数组, srcElement: 0, srcMip: i, dst: UV纹理数组, dstElement: 当前depth, dstMip: i);
                        }
                    }
                    else
                    {
                        UnityEngine.Graphics.CopyTexture(src: UV纹理数组, srcElement: 0, dst: UV纹理数组, dstElement: 当前depth);
                    }
    ...
    public class 游戏内置喷漆颜色_只读
    {
        ...
        {
            蓝色 = 0,
            灰色, 绿色, 橙色, 红色, 黄色, 白色, 黑色, 棕色, 卡其色, 粉色, 紫色,
        }
