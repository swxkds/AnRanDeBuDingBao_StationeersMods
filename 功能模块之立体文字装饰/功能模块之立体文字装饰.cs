    ...
    public class 功能模块之立体文字装饰 : BaseUnityPlugin
    {
        ...
        public static MultiConstructor 创建_套件_立体文字装饰()
        {
            ...
            通用工具.合并多边形网格([资源视图.查找资源<Mesh>("Assets/(套件)立体文字装饰/(套件)立体文字装饰_外壳网格.asset"), 资源视图.查找资源<Mesh>("Assets/(套件)立体文字装饰/(套件)立体文字装饰_内容网格.asset")], "(套件)立体文字装饰", Arg_保留子网格么: true),
             ...
             资源视图.查找所有喷漆颜色缩略图资源(缩略图资源目录, Arg_是否填充不存在颜色: false), 默认颜色);
            ...
            const int 挂墙小网格建筑模型计数 = 2;

            for (var i = 0; i < 挂墙小网格建筑模型计数; i++)
            {
                立体文字装饰 挂墙小网格建筑 = null;
                Mesh 模型网格 = null;
                ...
                switch (i)
                {
                    case 0:     // 立体文字装饰 平面0.5X0.5
                        {
                            const string NameID_1 = 立体文字装饰.外壳层级前缀;
                            const 通用工具.游戏内置喷漆颜色.色板 默认颜色1 = 通用工具.游戏内置喷漆颜色.色板.橙色;
                            const string 缩略图资源目录_1 = "Assets/立体文字装饰/所有喷漆颜色缩略图/";

                            var 外壳网格 = 资源视图.查找资源<Mesh>("Assets/立体文字装饰/立体文字装饰_外壳网格.asset");
                            var 灯泡网格 = 资源视图.查找资源<Mesh>("Assets/立体文字装饰/立体文字装饰_内容网格.asset");
                            模型网格 = 通用工具.合并多边形网格([外壳网格, 灯泡网格], "立体文字装饰", Arg_保留子网格么: false);

                            挂墙小网格建筑 = 通用工具.创建Thing预制体并进行通用初始化<立体文字装饰>
                            (通用工具.创建新的空预制体(), 通用工具.创建新的空预制体(), NameID_1 + i,
                            外壳网格, 通用工具.游戏内置喷漆颜色.游戏内置喷漆材质, 模型网格,
                            资源视图.查找所有喷漆颜色缩略图资源(缩略图资源目录_1, Arg_是否填充不存在颜色: false), 默认颜色1);
                    ...
                    case 1:     // 居住     平面0.5X1
                        {
                            const string NameID_1 = 立体文字装饰.外壳层级前缀;
                            const 通用工具.游戏内置喷漆颜色.色板 默认颜色1 = 通用工具.游戏内置喷漆颜色.色板.橙色;
                            const string 缩略图资源目录_1 = "Assets/居住/所有喷漆颜色缩略图/";

                            var 外壳网格 = 资源视图.查找资源<Mesh>("Assets/居住/居住_外壳网格.asset");
                            var 灯泡网格 = 资源视图.查找资源<Mesh>("Assets/居住/居住_内容网格.asset");
                            模型网格 = 通用工具.合并多边形网格([外壳网格, 灯泡网格], "居住", Arg_保留子网格么: false);
                ...
                }
                {
                    if (模型网格)
                    {
                        ...
                        结构损毁状态.AddComponent<MeshFilter>().sharedMesh = 模型网格;
