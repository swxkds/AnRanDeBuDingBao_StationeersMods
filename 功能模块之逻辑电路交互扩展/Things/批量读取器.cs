...
namespace meanran_xuexi_mods_xiaoyouhua
{
    ...
    {
        public static void 拧螺丝(this LogicBatchReader 批量读取器, Interactable 批量读取器控件, 通用可选择项目 已选择)
        {
            switch (已选择.解包标志)
            {
                case 通用可选择项目.数据解包标志.统计模式:
                    switch (批量读取器控件.Action)
                    {
                        case InteractableType.Button1:
                            拧螺丝一(批量读取器, (LogicBatchMethod)已选择.操作数["参数"]);
                            拧螺丝联机数据包.发送数据包(批量读取器.ReferenceId, 0, 批量读取器控件.InteractableId, 0, 已选择.操作数["参数"], 通用可选择项目.数据解包标志.统计模式);
                            break;
                        default: break;
                    }
                    break;
                case 通用可选择项目.数据解包标志.逻辑参数:
                    switch (批量读取器控件.Action)
                    {
                        case InteractableType.Button2:
                            拧螺丝二(批量读取器, (LogicType)已选择.操作数["参数"]);
                            拧螺丝联机数据包.发送数据包(批量读取器.ReferenceId, 0, 批量读取器控件.InteractableId, 0, 已选择.操作数["参数"], 通用可选择项目.数据解包标志.逻辑参数);
                            break;
                        default: break;
                    }
                    break;
                case 通用可选择项目.数据解包标志.物联网已上线设备:
                    switch (批量读取器控件.Action)
                    {
                        case InteractableType.Button3:
                            拧螺丝三(批量读取器, 已选择.链接物体);
                            拧螺丝联机数据包.发送数据包(批量读取器.ReferenceId, 已选择.链接物体.ReferenceId, 批量读取器控件.InteractableId, 0, 0, 通用可选择项目.数据解包标志.物联网已上线设备);
                            break;
                        default: break;
                    }
                    break;
        ...
        private static void 拧螺丝一(LogicBatchReader 批量读取器, LogicBatchMethod 统计类型)
        {
            ...
            ////批量读取器.aaaNetworkUpdateFlags |= aaaushort.MaxValue;
        }
        ...
        public static 通用可选择项目 生成消息(this LogicBatchReader 批量读取器, Interactable 批量读取器控件)
        {
            通用可选择项目 包 = new();
            ...
            switch (批量读取器控件.Action)
            {
                ...
                case InteractableType.Button3:
                    ...
                    包.物联网已上线设备表或内部储物表或试剂引用 = 批量读取器.InputNetwork1DevicesSorted?.Where(d => d != (ILogicable)批量读取器 && d.IsLogicReadable());
