...
namespace meanran_xuexi_mods_xiaoyouhua
{
    ...
    public class 功能模块之逻辑电路交互扩展 : BaseUnityPlugin
    {
        ...
        private IEnumerator 并发构造()
        {
            ...
            if (前置_资源加载器.单例.逻辑组件字典.TryGetValue(typeof(InputPrefabs), out var 选择配方面板))
            {
                ...
                {
                    补丁.UnpatchAll();
                    Log.LogMessage("功能模块之逻辑电路交互扩展_构造选择面板失败");
                }
                else
                {
                    var 面板标题 = inputPrefabs.TitleText;
                    面板标题.font = 前置_资源加载器.单例.当前TMP字体;
                    功能模块之逻辑电路交互扩展工具.修改字体尺寸(面板标题, 23);
                    ...
                    功能模块之逻辑电路交互扩展工具.修改字体尺寸(取消按钮, 23);
                    ...
                    功能模块之逻辑电路交互扩展工具.修改字体尺寸(输入框输入显示, 22);

                    var 输入框淡色提示 = (TMP_Text)搜索栏.placeholder;
                    输入框淡色提示.font = 前置_资源加载器.单例.当前TMP字体;
                    功能模块之逻辑电路交互扩展工具.修改字体尺寸(输入框淡色提示, 22);

                    var 配方大类标题 = inputPrefabs.ControlGroupPrefab.Title;
                    配方大类标题.font = 前置_资源加载器.单例.当前TMP字体;
                    功能模块之逻辑电路交互扩展工具.修改字体尺寸(配方大类标题, 29);

                    var 配方描述 = inputPrefabs.PrefabReference.Text;
                    配方描述.font = 前置_资源加载器.单例.当前TMP字体;
                    功能模块之逻辑电路交互扩展工具.修改字体尺寸(配方描述, 27);

                    var 新 = UnityEngine.Object.Instantiate(inputPrefabs.gameObject, inputPrefabs.transform.parent, false);

                    新.gameObject.name = "选择面板";
                    选择面板.单例 = 新.gameObject.AddComponent<选择面板>();
                    var 旧 = 新.GetComponent<InputPrefabs>();

                    选择面板.单例.面板标题 = 旧.TitleText;
                    选择面板.单例.面板标题.text = "选择面板";

                    选择面板.单例.内容区垂直布局组预制体 = 旧.GroupParents;
                    选择面板.单例.内容区垂直布局组预制体.name = "选择面板分支布局预制体";

                    选择面板.单例.内容区垂直布局组父级 = 选择面板.单例.内容区垂直布局组预制体.parent;
                    选择面板.单例.内容区垂直布局组预制体.SetParent(null, false);
                    选择面板.单例.内容区垂直布局组预制体.gameObject.SetActive(false);

                    for (int i = 选择面板.单例.内容区垂直布局组预制体.childCount - 1; i >= 0; i--)
                    { UnityEngine.Object.DestroyImmediate(选择面板.单例.内容区垂直布局组预制体.GetChild(i).gameObject); }     // 预制体自带一些大类层级需要删除掉
                    ...
                    var 新__ = __.gameObject.AddComponent<可选择项目>();
                    ...
                    选择面板.单例.可选择项目预制体 = 新__.gameObject;
                    选择面板.单例.可选择项目预制体.name = "可选择项目预制体";

                    选择面板.单例.面板搜索栏 = 旧.SearchBar;
                    选择面板.单例.UiComponentRenderer = 旧.UiComponentRenderer; // 等于null
                    选择面板.单例.GameObject = 旧.GameObject;       // 本级,这三个变量指向同一个层级
                    选择面板.单例.Transform = 旧.Transform;         // 本级,这三个变量指向同一个层级
                    选择面板.单例.RectTransform = 旧.RectTransform; // 本级,这三个变量指向同一个层级

                    选择面板.单例.关闭面板按钮 = 选择面板.单例.transform.GetChild(0).GetChild(4).GetComponent<Button>();
                    选择面板.单例.滚动组件 = 选择面板.单例.transform.GetChild(0).GetChild(7).GetComponent<ScrollRectNoDrag>();

                    UnityEngine.Object.Destroy(旧);
                    选择面板.单例.Initialize();

                    Log.LogMessage($"功能模块之逻辑电路交互扩展_构造选择面板成功");
                }
            }
        }
    }

    public class 可选择项目 : UserInterfaceAnimated, IScreenSpaceTooltip
    {
        public 数据包 数据包;
        public string DisplayName => 数据包.包头 == 数据包.数据解包标志.物联网已上线设备 ? 数据包.链接物体.DisplayName : 右侧文本.text;
        ...
        public void 复用初始化(数据包 包)
        {
            if (数据包 == null) { 数据包 = 数据包.拷贝构造(ref 包); }
            else { 数据包.拷贝赋值(ref 包, ref 数据包); }

            if (数据包.包头 == 数据包.数据解包标志.物联网已上线设备)
            {
            ...
            else
            {
                switch (数据包.包头)
                {
                    case 数据包.数据解包标志.试剂参数:
                        右侧文本.text = ((Reagent)数据包.物联网已上线设备表或试剂引用).DisplayName + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.逻辑参数:
                        右侧文本.text = Enum.GetName(typeof(LogicType), 数据包.操作数["参数"]) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.插槽参数:
                        右侧文本.text = EnumCollections.LogicSlotTypes.GetName((LogicSlotType)数据包.操作数["参数"], false) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.插槽编号:
                        右侧文本.text = 数据包.链接物体.GetSlot(数据包.操作数["插槽编号"]).DisplayName + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.试剂模式:
                        右侧文本.text = Enum.GetName(typeof(LogicReagentMode), 数据包.操作数["参数"]) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.统计模式:
                        右侧文本.text = Enum.GetName(typeof(LogicBatchMethod), 数据包.操作数["参数"]) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.基础数学运算符:
                        右侧文本.text = LogicMath.EnumOperators.GetName((MathOperators)数据包.操作数["参数"], false) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.高等数学运算符:
                        右侧文本.text = LogicMathUnary.EnumOperators.GetNameFromValue(数据包.操作数["参数"], false) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.比较运算符:
                        右侧文本.text = Enum.GetName(typeof(ConditionOperation), 数据包.操作数["参数"]) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.逻辑门运算符:
                        右侧文本.text = Enum.GetName(typeof(GateOperators), 数据包.操作数["参数"]) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.最大最小值运算符:
                        右侧文本.text = Enum.GetName(typeof(数据包.ComparisonOperation), 数据包.操作数["参数"]) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                    case 数据包.数据解包标志.物联网信号模式:
                        右侧文本.text = Enum.GetName(typeof(数据包.信号模式), 数据包.操作数["参数"]) + "\n" + Localization.GetSlotTooltip(Slot.Class.None); break;
                }

                左侧缩略图.sprite = 数据包.链接物体 ? 数据包.链接物体.Thumbnail : 数据包.主物体.Thumbnail;
            }
        }
    ...
    public class 数据包
    {
        ...
        public 数据包() { 操作数 = new(); }
        public 数据包(数据解包标志 包头, Thing 主物体, Interactable 控件, Thing 链接物体, object 物联网已上线设备表或试剂引用, params (string, int)[] 操作数) : this()
        {
            this.包头 = 包头;
            this.主物体 = 主物体;
            this.控件 = 控件;
        ...
        public static void 拷贝赋值(ref 数据包 源, ref 数据包 目标)
        {
            目标.包头 = 源.包头;
            目标.主物体 = 源.主物体;
            目标.控件 = 源.控件;
        ...
        public static 数据包 拷贝构造(ref 数据包 源)
        {
            var 新包 = new 数据包(源.包头, 源.主物体, 源.控件, 源.链接物体, 源.物联网已上线设备表或试剂引用);
            新包.操作数.AddRange(源.操作数);
            return 新包;
        }
    }

    public static class 功能模块之逻辑电路交互扩展工具
    {
        public static void 修改字体尺寸(TMP_Text component, float fontSize)
        {
            if (component == null)
            {
                功能模块之逻辑电路交互扩展.Log.LogDebug($"TMP_Text是空引用,无法修改字体尺寸");
                return;
            }
        ...
        public static void 修改字体尺寸(UnityEngine.UI.Text component, int fontSize)
        {
            if (component == null)
            {
                功能模块之逻辑电路交互扩展.Log.LogDebug($"Text是空引用,无法修改字体尺寸");
                return;
            }
